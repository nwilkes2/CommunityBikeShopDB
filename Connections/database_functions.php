...
function list_projects_mechanics($form_name = "none", $default_value = ""){
	$querySQL = "SELECT project_id FROM projects WHERE active = 1 ORDER BY mechanic DESC, project_id";
	$list_value = "project_id";
	$list_text = "project_id";
	generate_list($querySQL,$list_value,$list_text,$form_name, $default_value);
}
...
//
function formatnum($value, $currency , $places = 2){
	if($currency == 1) echo "$ ";	
